    ...
    <div class="w-full xl:w-8/12 mb-8 xl:mb-0 px-4 mx-auto mt-15">
        ...
        <div class="relative flex flex-col min-w-0 break-words bg-[#FEFEFE] w-full mb-6 shadow-lg rounded p-4 ">
            ...
            <div>
                ...
                <div>
                    <div class="overflow-hidden rounded-full bg-gray-200">
                        <div class="h-2  ${sessionScope.step==1?'w-1/6':sessionScope.step==2 ?'w-3/6':sessionScope.step==3?'w-4/6':'w-full'}  rounded-full  bg-[#1E4584]"></div>
                    </div>
                    ...
                    <ol class="mt-4 grid grid-cols-4 text-sm font-medium text-[#1E4584] ">
                        ...
                        <li class="flex items-center ${sessionScope.step!=2?'text-[#1E4584]':'text-[#1FAAAC]'} justify-end ">
                        ...
                        <li class="flex items-center justify-end  ${sessionScope.step!=3?'text-[#1E4584]':'text-[#1FAAAC]'}">
            ...
            <c:if test="${sessionScope.step==2 || sessionScope.step==3}">
                <div class="flex flex-1 mt-2 mx-3 items-center justify-end md:justify-between">
                    <h4 class="text-xl mt-3 font-bold sm:text-xl">
                        Totale ${sessionScope.step==2?'Medicament':'Document' }
                        ajouter
                        : ${sessionScope.step==2?dossierInfo.consultationPrisByIdConsultation.medicamentPrisesById.size():dossierInfo.consultationPrisByIdConsultation.documentPrisesById.size()} </h4>
                        <%--                        ${dossierInfo.}--%>
                    <h4 class="text-xl mt-3 font-bold sm:text-xl"> Mentant Rembourser
                        : ${mentant}DH </h4>
...
<%--                        <h1 class="text-2xl font-bold sm:text-3xl"> le mentant rembourssable est ${mentant}DH </h1>--%>
                        <div class="bg-white mx-auto  p-8 rounded-xl shadow-lg shadow-neutral-200 w-96">
                            <!-- header -->
                            <div class="flex justify-between mb-4">

                                <div>
<%--                                    <h1 class="text-2xl  font-bold sm:text-3xl"> ${sessionScope.step==1?'Patient Info':sessionScope.step==2?'Medicament : ':sessionScope.step==3?'Document : ':' Validation Dossier'} </h1>--%>
                                    <p class="text-lg font-semibold text-neutral-700">le mentant rembourssable</p>
                                </div>
                                <span class="text-lg text-neutral-600">${mentant}DH</span>
                            ...
                            <div class="mt-5 border-t border-dashed space-y-4 py-4">
                                ...
                                <div class="flex justify-between group duration-150 cursor-pointer">
                                    <div>
                                        <p class="text-lg text-neutral-600 group-hover:text-red-600 duration-150">Total  des Médicaments</p>
                                    </div>
                                    <span class="text-lg text-neutral-600">${dossierInfo.consultationPrisByIdConsultation.medicamentPrisesById.size()}</span>
                                </div>

                                <!-- item 2 -->
                                <div class="flex justify-between group duration-150 cursor-pointer">
                                    <div>
                                        <p class="text-lg text-neutral-600 group-hover:text-red-600 duration-150">Total  des Document</p>
                                    </div>
                                    <span class="text-lg text-neutral-600">${dossierInfo.consultationPrisByIdConsultation.documentPrisesById.size()}</span>
                                </div>
            ...
            <c:if test="${sessionScope.step!=1}">
                <form action="/dossier/next" method="post"
                      class="flex flex-1 mt-2 mx-3 items-center  ${(sessionScope.step==4)?'justify-start':'justify-end'}   ${(sessionScope.step==3)?'md:justify-between':''} ">
                    ...
                    <c:if test="${(sessionScope.step==3) ||(sessionScope.step==4)}">
                        ...
                        <span
                                class="  flex-shrink-0 rounded-full border border-indigo-600 bg-white p-2 group-active:border-indigo-500"
                        >


                <svg class="h-5 w-5"
                     xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.2.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path
                        d="M109.3 288L480 288c17.7 0 32-14.3 32-32s-14.3-32-32-32l-370.7 0 73.4-73.4c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-128 128c-12.5 12.5-12.5 32.8 0 45.3l128 128c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.3 288z"/></svg>  </span>
                            <span class=" ml-4 font-medium text-[#1E4584] transition-colors group-hover:text-white">
                    retour à l'étape avant
                    </span>
